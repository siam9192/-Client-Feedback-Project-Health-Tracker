...
function PendingCheckinCard({ checkin: check }: Props) {
  return (
    <div key={check._id} className="flex flex-col  p-3 rounded-md border border-gray-200">
      <div className="flex  flex-col gap-2 lg:gap-0  lg:flex-row justify-between lg:items-center">
        ...
        <span
          className={`text-xs md:text-sm font-semibold p-2 rounded-full size-fit ${getProjectStatusColor(
            check.status,
          )}`}
        >
      ...
      <div className="mt-1 text-sm text-gray-500 font-medium font-secondary">
        <div className="space-y-3">
          ...
          <div className="w-full bg-gray-200 rounded-full h-3 max-w-sm">
            <div
              ...
              style={{ width: `${check.progressPercentage}%` }}
            />
          </div>
          <span className="text-lg text-gray-600">{check.progressPercentage}%</span>
        </div>
      </div>
